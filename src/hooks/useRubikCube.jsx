...
export const useRubikCube = () => {
  ...
  const stepOne = () => {
    let temp = cube.map((face) => face.map((row) => row.map((carre) => carre)));
  ...
  const whiteCross = (rubikCube) => {
    if (
      rubikCube[0][0][1] === "white" &&
      rubikCube[0][1][0] === "white" &&
      rubikCube[0][1][1] === "white" &&
      rubikCube[0][1][2] === "white" &&
      rubikCube[0][2][1] === "white"
    ) {
      return true;
    }
    return false;
      ...
      for (let j = 0; j < 3; j++) {
        for (let k = 0; k < 3; k++) {
          if (temp[faces[i]][j][k] === "white") {
            faceConnected[`T_${faces[i]}_${j}_${k}`] &&
              (temp = stepTwo(temp, `T_${faces[i]}_${j}_${k}`));
            faceConnected[`D_${faces[i]}_${j}_${k}`] &&
              (temp = stepTwo(temp, `D_${faces[i]}_${j}_${k}`));
          }
        }
      }
    }
  ...
  const stepTwo = (temp, locationCarre) => {
    ...
    const cornerTwo =
      temp[colorAreteTwo[0]][colorAreteTwo[1]][colorAreteTwo[2]];
  ...
  const rearrangeCorner = (temp) => {
    for (let i = 0; i < cornersWhite.length; i++) {
      ...
      if (
        temp[+carreOne[0]][1][1] !== colorOne ||
        temp[+carreTwo[0]][1][1] !== colorTwo
      ) {
  ...
  const StartAlgoStepThree = (temp) => {
    ...
    for (let k = 0; k < facesArray.length; k++) {
      ...
      const color = colorArray.find(
        (element) => element === temp[faceCube][i][j]
      );
      if (color && temp[2][yellow[0]][yellow[1]] !== "yellow") {
        ...
        if (
          temp[2][y[0]][y[1]] === temp[facesArray[rightFace].faceCube][1][1]
        ) {
          moves = moveStepThree[`${face}_right`].split(" ");
        } else if (
          temp[2][y[0]][y[1]] === temp[facesArray[leftFace].faceCube][1][1]
        ) {
  ...
  const noGoodMoves = (temp) => {
    ...
    for (let k = 0; k < facesArray.length; k++) {
      ...
      if (color) {
        ...
        if (direction) {
          const firstMove = moveRightFace[color][faceCube];
          moves = [firstMove].concat(
            moveStepThree[`${face}_${direction}`].split(" ")
          );
          break;
        }
      }
  ...
  const checkWrongPosition = (temp) => {
    for (let i = 0; i < colorArray.length; i++) {
      ...
      const { face: secondFace, left } = moveRightFace[
        colorArray[(i + 1) % colorArray.length]
      ];
      if (
        temp[face][right[0]][right[1]] === temp[secondFace][1][1] &&
        temp[secondFace][left[0]][left[1]] === temp[face][1][1]
      ) {
        const moves = [].concat(
          moveStepThree[`${face}_right`].split(" "),
          "B2",
          moveStepThree[`${face}_right`].split(" ")
        );
        temp = makeMoves(temp, moves);
  ...
  const secondCrowned = (temp) => {
    for (let i = 0; i < colorArray.length; i++) {
      ...
      if (
        temp[face][left[0]][left[1]] !== temp[face][1][1] ||
        temp[face][1][1] !== temp[face][right[0]][right[1]]
      ) {
        return false;
      }
    }
  ...
  const checkFirstCase = (temp) => {
    ...
    for (let m = 0; m < listMoves.length; m++) {
      ...
      for (let k = 0; k < facesArray.length; k++) {
        const { faceCube, i, j } = facesArray[k];
        const { faceCube: prevFaceCube } = facesArray[
          selectIndex(listMoves[m], k)
        ];
  ...
  const checkSecondCase = (temp) => {
    ...
    if (inversePosition(temp)) {
      for (let k = 0; k < facesArray.length; k++) {
        const { faceCube, i, j } = facesArray[k];
        const { faceCube: secondFace, i: secondI, j: secondJ } = facesArray[
          (k + 2) % facesArray.length
        ];
        if (
          temp[faceCube][i][j] === temp[secondFace][1][1] &&
          temp[secondFace][secondI][secondJ] === temp[faceCube][1][1]
        ) {
    ...
    for (let k = 0; k < facesArray.length; k++) {
      const { faceCube, i, j } = facesArray[k];
      const { faceCube: secondFace, i: secondI, j: secondJ } = facesArray[
        (k + 1) % facesArray.length
      ];
      const { faceCube: thirdFace } = facesArray[
        (k + facesArray.length - 1) % facesArray.length
      ];
      if (
        temp[secondFace][secondI][secondJ] === temp[secondFace][1][1] &&
        temp[faceCube][i][j] === temp[thirdFace][1][1]
      ) {
        moves = moveStepFive[faceCube].split(" ");
        temp = makeMoves(temp, moves);
      }
  ...
  const inversePosition = (temp) => {
    for (let k = 0; k < facesArray.length; k++) {
      ...
      if (
        temp[faceCube][i][j] === temp[faceCube][1][1] &&
        temp[secondFace][secondI][secondJ] === temp[secondFace][1][1]
      ) {
        return true;
      }
    ...
    for (let i = 0; i < cornersYellow.length; i++) {
      ...
      if (checkCorner(temp, carreOne, carreTwo, carreThree)) {
        if (temp[+carreOne[0]][+carreOne[1]][+carreOne[2]] === "yellow") {
          ...
          temp[+carreTwo[0]][+carreTwo[1]][+carreTwo[2]] === "yellow"
        ) {
  ...
  const chooseMove = (cornerOne, cornerTwo) => {
    if (
      (cornerOne === "green" && cornerTwo === "red") ||
      (cornerOne === "red" && cornerTwo === "green")
    ) {
      return 5;
    } else if (
      (cornerOne === "blue" && cornerTwo === "red") ||
      (cornerOne === "red" && cornerTwo === "blue")
    ) {
      return 3;
    } else if (
      (cornerOne === "green" && cornerTwo === "orange") ||
      (cornerOne === "orange" && cornerTwo === "green")
    ) {
      return 1;
    } else if (
      (cornerOne === "orange" && cornerTwo === "blue") ||
      (cornerOne === "blue" && cornerTwo === "orange")
    ) {
      return 4;
    }
  };
    ...
    for (let i = 0; i < cornerFace.length; i++) {
      for (let j = 0; j < colorCornerArray.length; j++) {
        if (temp[cornerFace[i]][1][1] === colorCornerArray[j]) {
          colorCornerArray = colorCornerArray.filter(
            (element) => element !== colorCornerArray[j]
          );
          break;
        }
      }
  ...
  const makeMoves = (temp, moves) => {
    moves.forEach((move) => {
      ...
      setListAllMoves((prevState) =>
        move ? [...prevState, move] : [...prevState]
      );
    });
    return temp;
  };
  ...
  const shuffleCube = () => {
    ...
    for (let i = 0; i < randomMovesArray.length; i++) {
      setRubikCube((prevState) =>
        updateRubikCube(prevState, randomMovesArray[i])
      );
    }
  };
