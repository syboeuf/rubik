export const useViewer = () => {
    ...
    const rotateFaceCube = (temp, faceCube, des, rubikCube) => {
        let {
            face, i, j, increase, one, two,
        } = faceCube
        for (let k = 0; k < 3; k++) {
            temp[des - 1][i][(one > -1) ? one : k] = rubikCube[face - 1][j][(two > -1) ? two : k]
    ...
    const startSimpleMove = (temp, faceCube, rubikCube) => {
        for (let i = 0; i < faceCube.length; i++) {
            temp = rotateFaceCube(temp, faceCube[i], faceCube[(i + 1) % faceCube.length].face, rubikCube)
        }
        return temp
    }
        ...
        for (let l = 0; l < faceCube.length; l++) {
            ...
            for (let k = 0, m = 2; k < 3; k++, m--) {
                if (direction === "up" || direction === "down") {
                    ...
                    temp[des - 1][i][(one > -1) ? one : m] = rubikCube[face - 1][j][(two > -1) ? two : k]
                }
                increase.indexOf("j+") > -1 && j++
                increase.indexOf("j-") > -1 && j--
                increase.indexOf("i+") > -1 && i++
